...
public class SearchServiceImpl {
    ...
    public Page<PostIndex> simpleSearch(List<String> keywords, Pageable pageable) {
        var searchQueryBuilder = new NativeQueryBuilder().withQuery(buildSimpleSearchQuery(keywords)).withPageable(pageable);
    ...
    private Query buildSimpleSearchQuery(List<String> tokens) {
        return BoolQuery.of(q -> q.must(mb -> mb.bool(b -> {
            tokens.forEach(token -> {
                ...
                b.should(sb -> sb.match(m -> m.field("full_content").query(token)));
                b.should(sb -> sb.match(m -> m.field("file_content").query(token)));
//                b.should(sb -> sb.match(m -> m.field("number_of_likes").query(token)));
                b.should(sb -> sb.match(m -> m.field("comment_content").query(token)));

                try {
                    Long numberOfLikes = Long.parseLong(token);
    ...
    private Query buildAdvancedSearchQuery(List<String> operands, String operation) {
        return BoolQuery.of(q -> q.must(mb -> mb.bool(b -> {
            ...
            switch (operation) {
                case "AND":
                    b.must(sb -> sb.match(m -> m.field(field1).fuzziness(Fuzziness.ONE.asString()).query(value1)));
                    b.must(sb -> sb.match(m -> m.field(field2).query(value2)));
                    break;
                case "OR":
                    b.should(sb -> sb.match(m -> m.field(field1).fuzziness(Fuzziness.ONE.asString()).query(value1)));
    ...
    private Page<PostIndex> runQuery(NativeQuery searchQuery) {
        var searchHits = elasticsearchTemplate.search(searchQuery, PostIndex.class, IndexCoordinates.of("posts_index"));
